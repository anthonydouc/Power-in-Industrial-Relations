...
from .lookups import (get_journal_reps_contains, get_journal_reps,
                      ...
                      get_journal_attrs, get_scraped_paper_data)
...
def check_data_cols(data: pd.DataFrame) -> pd.DataFrame:
    ...
    a_gender_columns = [f'Gender {x}' for x in range(1, 10)]
    
    a_country_columns = [f'Author {x} country' for x in range(1, 10)]
    
    a_work_columns = [f'Author {x} work' for x in range(1, 10)]
    ...
    savecolumns = ['Standard error', 'Effect size', 'Journal', 'Filter',
                   ...
                   'Acceptance date', 'Revised date', 'Month published',
                   "Revised rather than accepted date",
                   'Author share of estimates', 
                   'Google scholar citations may 2021', 'Title',  'Doi',
                   'Data available', 'Four years prior',	'Same year', 'Three years after',
                   'Four years prior top 5', 'Same year top 5', 'Three years after top 5']

    savecolumns += authorcolumns 
    
    savecolumns += a_gender_columns
    
    savecolumns += a_country_columns
    
    savecolumns += a_work_columns
...
def compile_data(readdir:str, keep_journals: list=None, drop_for: list=None,
    ...
    for file in files:
        ...
        if len(np.unique(data.columns)) != len(data.columns):
            print(file)
            print(data.columns)
    ...
    data.to_excel(f'{readdir}all_data.xlsx', sheet_name='Data')
    
    ValueError("SAVED MATE")
...
def author_reps(data: pd.DataFrame) -> pd.DataFrame:
    ...
    all_cols = authorcolumns + a_gender_columns + a_country_columns + a_work_columns
...
def assign_acceptance_time(data: pd.DataFrame) -> pd.DataFrame:
    ...
    date_fields = ['Acceptance date', 'Submission date', "Revised date",
                   "Month published"]
    ...
    for date_field in date_fields:
        ...
        data[f'{date_field} parsed'] = pd.NaT
    ...
    for date_field in date_fields:
        ...
        for fmt in formats:
            ...
            data.loc[mnull, fld_parsed] = pd.to_datetime(data.loc[mnull, fld_parsed],
                                                         format='%Y-%d-%m',
                                                         errors='coerce')
        data[date_field] = data[f'{date_field} parsed']
    ...
    data["Submit_to_revision_days"] = (data["Revised date"] - data['Submission date']).dt.days

    data['Submit_to_revision_weeks'] = data['Submit_to_revision_days'] / 7
   
    data["Revise_to_accept_days"] = ( data["Acceptance date"] - data['Revised date']).dt.days

    data["Revise_to_accept_weeks"] = data["Revise_to_accept_days"] / 7
    
    data["Accept_to_publish_days"] = (data['Month published'] - data['Acceptance date']).dt.days
    
    data["Accept_to_publish_weeks"] = data["Accept_to_publish_days"] / 7
    
    data["Submit_to_publish_days"] = (data['Month published'] - data['Submission date']).dt.days
    
    data["Submit_to_publish_weeks"] = data["Submit_to_publish_days"] / 7
    
    data["Revised_not_accepted"] = 0
    
    data.loc[data["Revised rather than accepted date"] == "YES", "Revised_not_accepted"] = 1

    data['First_dom_acceptance_and_submission'] = (data['Acceptance date'].dt.day == 1
                                                   & (data['Submission date'].dt.day == 1)).astype(int)

    data = data.drop(['Acceptance date parsed',
                      'Submission date parsed',
                      "Revised date parsed",
                      "Month published parsed"], axis=1)
...
def assign_paper_stats(data):
    paper_stats = get_scraped_paper_data()
    
    data = pd.merge(data, paper_stats[['Filename', 'Study no.', 'n_citations', 'issue', 'first_page', 'last_page', 'n_pages']], how='left', on=['Filename', 'Study no.'])
    return data
...
def calc_gender_ratio(data):

    a_gender_columns = [f'Gender {x}' for x in range(1, 10)]

    data['N female authors'] = (data[a_gender_columns] == "Female").sum(axis=1)

    data['N male authors'] = (data[a_gender_columns] == "Male").sum(axis=1)

    data['Prop female authors'] = (data['N female authors'] / data['Number of authors']).fillna(0)
...
def post_process_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # calculate author gender stats.
    data = calc_gender_ratio(data)
    
    # assign number of citations, number of pages, issue number
    data = assign_paper_stats(data)
...
if __name__ == "__main__":
    ...
    from poweranalysis import get_all_data
    readdir = '/Users/ad/Documents/Github/Poweranalysis/data/group1/'
    
    d = get_all_data(readdir)
    
    dm = d[d["Month published"].notnull()]
