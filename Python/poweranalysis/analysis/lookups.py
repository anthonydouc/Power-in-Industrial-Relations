...
def read_editors() -> pd.DataFrame:
    ...
    editors['Gender'] = editors['Gender'].str.lower()
...
def get_study_type() -> dict:
    #with open(f'{infodir}/Meta-Studies Included 16 Feb 2020.xlsx', mode="r", encoding="latin-1") as file:
    #    meta_data = pd.read_excel(file)
    meta_data = pd.read_excel(f'{infodir}/Meta-Studies Included 16 Feb 2020.xlsx', engine='openpyxl')#, encoding='latin-1')
...
def get_scraped_paper_data():
    
    data = pd.read_csv(f'{infodir}/citation_counts_by_doi.csv')
