...
def calc_windowed_regressions(data: pd.DataFrame, how: str='wls'):
    if how in ['rfe']:
        return ValueError(f'Method {how} not yet implemented for rolling regressions.')

    years = data.loc[data['yearsubmitted'].notnull(), 'yearsubmitted'].unique()

    all_res = {}

    for year in years:

        data_flt = data[data['Year published'] < year]

        vals = calc_and_extract_regressions(data_flt, how)

        all_res[year] = vals

    all_res['All'] = calc_and_extract_regressions(data, how)
...
def calc_and_extract_regressions(data: pd.DataFrame, how: str='wls'):
    
    if len(data) == 1:
        vals = (data['Effect size'].values[0], np.nan)
    elif len(data) == 0:
        vals = (np.nan, np.nan)
    else:
...
def calc_estimates(data: pd.DataFrame, how: str='wls') -> pd.Series:
    ...
    vals = calc_windowed_regressions(data, how)

    return pd.Series({(f'{cols[i]}_{how}', year): val for year, res in vals.items() for i, val in enumerate(res)})
