...
def map_editors(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for etype in etypes:
        ...
        data_merge[f'N_Female_{etype.lower()}'] = data_merge['Gender'].str.contains('female') == True
   
        names = (data_merge[data_merge['Gender'].fillna('').str.contains('female')]
                 .groupby('Obs id')
                 .agg({'Name': lambda x: list(x.unique())})
                 .rename(columns={'Name':f'Female_{etype.lower()}'}))
        ...
        data = data.merge(names, on=['Obs id'], how='left')


    data.loc[data['Female_editor'].isnull(), 'Female_editor'] = data.loc[data['Female_editor'].isnull(), 'Female_editor'].apply(lambda x: [])
    
    data.loc[data['Female_co-editor'].isnull(), 'Female_co-editor'] = data.loc[data['Female_co-editor'].isnull(), 'Female_co-editor'].apply(lambda x: [])

    data['Unique_female_editors'] = (data['Female_editor'] + data['Female_co-editor']).apply(lambda row: list(set(row)))

    data['N_Female_editors'] = data['Unique_female_editors'].map(len)
    
    data['Female_editor_or_coeditor_present'] = (data['N_Female_editors'] >= 1).astype(int)
...
def editor_changes(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Proportion of female editors
    data['Prop of femail editors'] = data['N_Female_editors'] / data['Number of unique editors']
